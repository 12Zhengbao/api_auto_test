...
class NewProduct:
    ...
    def product_submission(self, get_api_headers):
        ...
        try:
            ...
            # assert rep.json().get('msg') == '', logger.info(f'断言不通过，响应内容为：f{rep.json().get('msg')}')
            # print(rep.json())
...
def run_stress_test():
    ...
    stats = {
        ...
        "failed_cases": [],
        "response_times": []  # 存储每次成功请求的响应时间（用于画图）
    }
...
<body>
    <div class="report-wrapper">
        ...
        <!-- 核心指标 -->
        <div class="section">
            <div class="section-title">核心测试指标</div>
        ...
        <!-- 详细统计 -->
        <div class="section">
            <div class="section-title">详细响应统计</div>
            <table>
                <tr>
                    <th>统计项</th>
                    <th>数值</th>
                ...
                <tr>
                    <td>测试总耗时</td>
                    <td>{total_elapsed} 秒</td>
                </tr>
                <tr>
                    <td>最大响应耗时</td>
                    <td>{stats['max_elapsed_ms']} 毫秒</td>
                </tr>
                <tr>
                    <td>最小响应耗时</td>
                    <td>{stats['min_elapsed_ms']} 毫秒</td>
                </tr>
                <tr>
                    <td>总响应耗时</td>
                    <td>{stats['total_elapsed_ms']} 毫秒</td>
                </tr>
            </table>
        </div>
    ...
    <script>
        ...
        new Chart(lineCtx, {{
            type: 'line',
            data: {{
                labels: {list(range(1, len(stats['response_times'])+1))},  // 请求序号
                datasets: [{{
                    label: '响应耗时(ms)',
    ...
    print("\n" + "="*60)
    print("压测完成！汇总结果：")
    print(f"  成功率：{success_rate}% | 平均响应耗时：{avg_elapsed}ms")
